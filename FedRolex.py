...
class Client_Rolex(Client):
    def __init__(self, mask_rate=1, *args, **kwargs):
...
if __name__ == "__main__":
    ...
    for client_id in range(args.nusers):
        cl = Client_Rolex(clients_roll_rate[client_id], config, device, client_id, dataset_train[client_id], dataset_val[client_id],
    ...
    for round in range(args.rounds):
        clients_roll_rate = make_model_rate(config, clients_roll_rate)
        upload_cost_round = []
        download_cost_round = []
        compute_flops_round = []
