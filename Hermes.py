    ...
    for key in global_weights.keys():
        if key in config.mask_weight_indicator:
            ...
            for i in range(1, len(masks)):
                mask += masks[i][step]
            global_weights[key] = torch.tensor(
...
def mask_model(model, mask):
    ...
    for name, param in model.named_parameters():
        if name in model.mask_weight_indicator:
            ...
            param.data = torch.tensor(mask[step] * param.data.cpu().numpy()).to(weight_dev)
            if param.data.dtype is not torch.float32:
...
class Client:
    ...
    def prune_by_percentile(self):
        ...
        for name, param in self.local_net.named_parameters():
            ...
            if name in self.local_net.mask_weight_indicator:
                tensor = param.data
                weight_magnitude = torch.sum(torch.abs(tensor), list(range(1, tensor.dim())))
                alive = weight_magnitude[np.nonzero(weight_magnitude.cpu().numpy())]
                percentile_value = np.percentile(abs(alive.cpu().numpy()), self.args.prune_percent)

                # Convert Tensors to numpy and calculate
                weight_dev = param.device
                new_mask = torch.where(weight_magnitude < percentile_value, 0, 1).view(-1, 1)
                for j in range(2, tensor.dim()):
                    new_mask = new_mask.unsqueeze(dim=1)
                new_mask = new_mask.expand_as(tensor)

                # Apply new weight and mask
                param.data = (tensor * new_mask).to(weight_dev)
                self.mask[step] = new_mask.cpu().numpy()
                step += 1
    ...
    def loss(self, out, y):
        ...
        for name, param in self.local_net.named_parameters():
            if name in self.local_net.mask_weight_indicator:
                tensor = param.data
                weight_magnitude = torch.sum(torch.pow(tensor, 2), list(range(1, tensor.dim())))
                loss += self.args.lamda * torch.sum(torch.sqrt(weight_magnitude))
            return loss
    ...
    def train(self, w_server, round, lr=None):
        ...
        for iter in range(self.local_epochs):
            ...
            for batch_ind, local_data in enumerate(self.traindata_loader):
                ...
                for name, p in self.local_net.named_parameters():
                    if name in self.local_net.mask_weight_indicator:
                        p.grad.data = p.grad.data * torch.tensor(self.mask[step]).to(self.device)
                        step += 1
        ...
        train_flops = len(self.traindata_loader) * count_training_flops(copy.deepcopy(self.model_trans), self.args,
                                                                        full=False) * self.args.local_ep

        grad = {}
        step = 0
        for k, param in self.local_net.named_parameters():
...
if __name__ == "__main__":
    ...
    model_saved = './log/{}/model_Hermes_{}.pt'.format(args.dataset, args.seed)
    ...
    for client_id in range(args.nusers):
        cl = Client(config, device, client_id, dataset_train[client_id], dataset_val[client_id],
                    ...
                    local_net=all_models[args.dataset](config, device), initial_global_params=w_glob)
    ...
    try:
        for round in range(args.rounds):
            ...
            for _, c in clients.items():
                ...
                train_res = evaluate(config, c.traindata_loader, local_mask_model, device)
                train_loss.append(train_res[0])
                train_acc.append(train_res[1])
                per_val_res = evaluate(config, c.valdata_loader, local_mask_model, device)
            ...
            if not best_val_loss or sum(per_val_acc) / len(per_val_acc) > best_val_loss:
                with open(model_saved, 'wb') as f:
                    print('save model')
                    torch.save(net_glob, f)
                best_val_loss = sum(per_val_acc) / len(per_val_acc)

            glob_test_loss, glob_test_acc = [], []
            for _, c in clients.items():
                local_mask_model = copy.deepcopy(net_glob)
                mask_model(local_mask_model, c.mask)
                glob_test_res = evaluate(config, c.testdata_loader, local_mask_model, device)
                glob_test_loss.append(glob_test_res[0])
                glob_test_acc.append(glob_test_res[1])
            print("Personalized test Round {}, test loss: {:.5f}, test accuracy: {:.5f}".format(round,
    ...
    for _, c in clients.items():
        local_mask_model = copy.deepcopy(model_best)
        mask_model(local_mask_model, c.mask)
        per_test_res = evaluate(config, c.testdata_loader, local_mask_model, device)
