    ...
    for key in global_weights.keys():
        if key in config.mask_weight_indicator:
            ...
            for i in range(1, len(masks)):
                mask += masks[i][step]
            global_weights[key] = torch.from_numpy(np.where(mask<1, global_weights_last[key].cpu(), global_weights_epoch[key].cpu())).to(config.device)
            step += 1
...
class Client:
    ...
    def train(self, w_server, round, lr=None):
        ...
        train_flops = len(self.traindata_loader) * count_training_flops(copy.deepcopy(self.model_trans),
    ...
    try:
        for round in range(args.rounds):
            ...
            print("Personalized Round {}, Validation loss: {:.5f}, "
                  "val accuracy: {:.5f}".format(round, sum(per_val_loss) / len(per_val_loss),
                                                sum(per_val_acc) / len(per_val_acc)))
